    ...
    <div class="grid_column flex">
      <div class="w-1/6" v-for="(column, colIndex) in columns" :key="colIndex" :ref="bindCol(colIndex)">
        ...
        <div class="w-full" v-for="(row, rowIndex) in rows" :key="rowIndex">
          <input @keydown.down="cellDown" @keydown.up="cellUp" @keydown.left="cellLeft" @keydown.right="cellRight" @keydown.enter="enterCell" :data-col="colIndex" :data-row="rowIndex" @keydown.tab="tabCell" :ref="bindRow(rowIndex, colIndex)" type="text" class="focus:outline-none focus:bg-green-100 focus:border-gray-200 py-1 px-1 border border-gray-100 min-w-full">
        </div>
      </div>
    </div>
  </div>
