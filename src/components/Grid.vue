      ...
      <div v-for="(column, colIndex) in structure.columns" :key="colIndex" :ref="bindCol(colIndex)">
        <div class="bg-gray-100 max-h-8 flex text-sm border-b border-r border-gray-300 text-center select-none py-1 px-2 w-full">
          ...
          <div :ref="bindColMenu(colIndex)"
            ...
            <div>
              <button @click="renameColumn(colIndex)" class="px-3 text-left py-1 w-full focus:outline-none hover:bg-gray-600">Rename column</button>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      renameColumn(colIndex:number) {
        ...
        const popover:HTMLElement = this.$refs[`colMenu${colIndex}`] as HTMLElement
